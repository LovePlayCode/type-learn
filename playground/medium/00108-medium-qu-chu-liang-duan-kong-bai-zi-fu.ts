...
type Space = ' '|'\n'|'\t'
type Trim<S extends string> = S extends `${Space}${infer U}` ? Trim<U> : S extends `${infer K}${Space}` ? Trim<K> : S
type Trim1<S extends string> = S extends `${Space}${infer T}` | `${infer T}${Space}` ? Trim<T> : S
